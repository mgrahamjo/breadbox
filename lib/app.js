...
module.exports = (settings = {}) => {
  ...
  http.createServer((req, res) => {
    ...
    let parentDir = path.join(__dirname, '../..').split('/').pop(),
    ...
    function error(message, data, status) {
      ...
      render(__dirname.replace('/dist', '/views/error.html'), errorData, appRoutes['/error']).then(response => {
    ...
    if (isView && staticUrls.indexOf(relPath) === -1) {
      ...
      for (let i = 0; i < variableUrls.length; i++) {

        let keys = variableUrls[i].match(varRegx);
    ...
    function getTemplate() {
      ...
      render(filepath, request, controller).then((response, headers = {}) => {
