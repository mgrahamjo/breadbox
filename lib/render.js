...
function parseVars(template, context, match) {
    ...
    try {
        ...
        console.log('error at parseVars: ' + err);
        value = '';
    }
...
function parseLoops(template, context, match) {
    ...
    try {
        ...
        console.log('error at parseLoops: ' + err);
    }
...
function parseIfs(template, context, match) {
    ...
    try {
        ...
        console.log('error at parseIfs:' + err);
    }
...
function parseIncludes(template, callback) {
    ...
    if (match !== null) {
        ...
        read('././views/' + path + '.html', { encoding: 'utf8' }, function(err, html) {
...
function render(url, filepath, request) {
    ...
    getContext(url, request).then(function(context, customPath) {
        
        if (customPath) {
            filepath = __dirname.replace('/lib', '/views/') + customPath;
        }
