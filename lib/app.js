...
module.exports = function(settings) {
  ...
  http.createServer(function(req, res) {
    ...
    function parseVars(route, keys) {
      ...
      var pathMatches = new RegExp(route.replace(varRegx, '([\\S]*)').split('|')[0]).exec(relPath);
    ...
    function fourZeroFour(type) {

      console.log(type + ' not found: ' + path);

      res.writeHead(404, {'Content-Type': 'text/plain'});

      res.end(type + ' not found');
    }
    ...
    if (extension === '.html') {
      ...
      if (typeof controller === 'undefined') {
        ...
        if (typeof controller === 'undefined') {
          ...
          if (typeof controller === 'undefined') {
            fourZeroFour('controller');
          }
        }
      }
    }

    if (authenticate && pathname !== '/login') {
      login();
    }
    ...
    fs.exists(filepath, function(exists) {
      ...
      function getTemplate() {
        render(routeName.replace(/.html$/, ''), filepath, request, res, controller).then(function(response) {
          res.end(response);
        });
      }
