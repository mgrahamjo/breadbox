...
module.exports = function(controllers, port) {
   
  http.createServer(function(req, res) {
    // filepath is (eventually) the full path to the template
    var filepath = __dirname.replace('/node_modules/breadbox/lib', ''),
    ...
    if (typeof controllers[routeName] === 'undefined' && routeName.indexOf('/admin') === 0) {
      filepath = filepath.replace('/views', '/node_modules/breadbox/views');
    }
    ...
    fs.exists(filepath, function(exists) {
      ...
      function getTemplate() {
        ...
        render(routeName.replace(/.html$/, ''), filepath, request, controllers).then(function(response) {
          res.end(response);
        });
      }
